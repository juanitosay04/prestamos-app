...
import { FileText, ShieldAlert, Phone, MapPin, User, CheckCircle2 } from "lucide-react"
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                  Gestión de titulares, historial crediticio y control de riesgo.
                </p>
              </div>
                ...
                {clientes.length === 0 ? (
                  ...
                  clientes.map((cliente) => (
                    <div key={cliente.id} className="p-4 space-y-3.5 hover:bg-white/[0.02] transition-colors">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-muted-foreground font-mono">{cliente.idDocument}</p>
                          </div>
                        </div>
                      ...
                      <div className="flex justify-end items-center gap-2 pt-1 border-t border-white/[0.04]">
                        <BlacklistToggleButton clientId={cliente.id} isBlacklisted={cliente.isBlacklisted} />
                        <Link 
                          href={`/clientes/${cliente.id}`} 
                          className="p-2 text-muted-foreground hover:text-white bg-white/[0.04] hover:bg-white/[0.08] rounded-lg transition-colors" 
                        ...
                        <tr key={cliente.id} className="hover:bg-white/[0.03] transition-colors">
                          ...
                          <td className="px-6 py-4 text-right">
                            <div className="flex justify-end items-center gap-2">
                              ...
                              <Link 
                                href={`/clientes/${cliente.id}`} 
                                className="p-2 text-muted-foreground hover:text-white bg-white/[0.04] hover:bg-white/[0.08] rounded-lg transition-colors border border-white/[0.04]" 
